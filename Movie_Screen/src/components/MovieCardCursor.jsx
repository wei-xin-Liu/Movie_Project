						...
						{movieGroups.map((group, index) => (
							<div key={index} className='w-full flex-shrink-0 hidden md:block'>
								<div className='grid md:grid-cols-4 gap-4 p-8'>
									{group.map(
										({ MID, image, rating, title, e_title, release_date }) => (
											<a
												href={`/program/${MID}`}
												key={MID}
											...
											>
												...
												>
													...
													/>
													<img
														className='absolute right-0 w-12 p-2'
														src={rating.ratingimgurl}
														alt={rating.ratingdesc}
													/>
													<p className='text-black text-xl py-2 pl-3'>
														{title}
													</p>
													<div className='grow' />
													<p className='text-1xl pl-3'>{e_title}</p>
													<div className='content-center'>
														<p className='text-1xl pl-3'>
															上映日期：{release_date}
														</p>
													</div>
													<button
