    ...
    <div className="font-sans p-[2%]">
      <div className="grid grid-flow-row sm:grid-cols-[15%_85%]">
        ...
        <div className="ml-[2%] mt-[2%] sm:mt-[0%]">
          <h1>{title}</h1>
          <h1>{e_title}</h1>
          <div className="grid grid-cols-[20%_80%] sm:grid-cols-[5%_95%] gap-[1%] mt-[2%]">
            <h1>級別</h1><div><img className="w-[10%] md:w-[2%]" src={`/${rating.ratingimgurl}`} alt={rating.ratingiddesc} /></div>
            <h1>片長</h1><div>{movieLength}分鐘</div>
            <h1>上映日</h1><div>{release_date}</div>
            <h1>類型</h1><div>{genre}</div>
            <h1>演員</h1><div>{actor}</div>
            <h1>導演</h1><div>{director}</div>
            <h1>簡介</h1>
            <div>
              <p id="movieText" className={`mr-[5%] movieText ${isExpanded ? 'expanded' : ''}`}>
                {description}
              </p>
              <button
                id="btnmovieText"
