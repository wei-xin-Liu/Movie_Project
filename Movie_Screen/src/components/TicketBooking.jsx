...
const TicketBooking = () => {
  ...
  const [ticketCounts, setTicketCounts] = useState({ adult: 0, student: 0, early: 0, love: 0 });
  ...
  useEffect(() => {
    if (theater) {
      axios.post('http://localhost/Movie_Project/Movie/public/api/get-ticket-prices', { theaterName: theater }) // http://localhost:8000/api/get-ticket-prices
  ...
  // 獲取食物資訊
  useEffect(() => {
    axios.get('http://localhost/Movie_Project/Movie/public/api/foods') // http://localhost:8000/api/foods
      .then(response => {
  ...
  // 票與食物的總價
  const totalPrice = () => {
    const ticketTotal = ticketPrice.reduce((total, price) => (
      total +
    ...
    const foodTotal = foods.reduce((total, food) => (
      total + (foodCounts[food.FID] * food.price)
    ), 0);
  ...
  const ticketRow = (label, price, countKey) => (
    <div className="flex items-center justify-between mb-1">
      <span className="text-rose-800">{label} NT{price}</span>
      <select
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      >
        ...
        <TicketInfo label="地點" value={theater} />
      </div>
      ...
      <div
        ...
        `}
      >
        <h2 className="text-xl font-bold text-center text-gray-800">票種</h2>
        <ul>
      ...
      <div
        ...
        `}
      >
        <h2 className="text-xl font-bold text-center text-gray-800 pb-4">可選食物</h2>
